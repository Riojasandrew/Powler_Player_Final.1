...
3/23–3/24/2026
•	Improved song image display
•	All images now resize consistently and look clean
•	Added search system for music player
•	User can type to find songs
•	Program scans songs[] using .filter() and .includes()
•	Built dynamic search results
•	Results appear as clickable buttons
•	Connected with <div id="search_results">
•	Connected everything together
Overall, display song images for user and allows users to search for music for the player. This will scan the song list to match result.
March 25th
-	Researching how to improve AI interaction less robotic and more emotional interactive
March 26th Update:
•	Upgraded AOI 
•	Changed messages into multiple options 
•	Added a random selection so AOI doesn’t repeat to the user
March 30th Update:
Creating a popin method where machine learning will be add. For now building a bubble icon for user to click up to open and close bubble. First going to create a small clickable. Next hidden chat box should appear. Afterwards, create hidden chat to appear once user clicks icon. Next trying to connect animation to song once new song start animation resets. Method used was using keyframes for animation

April 2nd Update:
Created a chat box assistant that lets the user talk back and forth(still in the build process). It has a simple deign with a message area, an input text system, and a send button. The layout and everything lines up so the messages don’t stack on top of each other and the button send and text box don’t overlap.

April 3rd - 5th Update:
Connected it to the mood detection and responses system similar to the “How are you feeling today” text. Now it reads the user’s mood and replies with matching dialog. Music is also tied to the text, so recommended songs play based on that mood. The first suggested has a auto play to it now. Didn’t have it before!
